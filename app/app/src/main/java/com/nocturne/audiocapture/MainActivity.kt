...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnSessions.setOnClickListener { startActivity(Intent(this, SessionListActivity::class.java)) }
    ...
    override fun onResume() { super.onResume(); fetchStatus(); handler.postDelayed(pollRunnable, pollInterval) }
    override fun onPause() { super.onPause(); handler.removeCallbacks(pollRunnable) }
            ...
            try {
                ...
                withContext(Dispatchers.Main) {
                    binding.tvRecordingDot.setTextColor(
                        if (status.recording) ContextCompat.getColor(this@MainActivity, R.color.green_active)
                        else ContextCompat.getColor(this@MainActivity, R.color.grey_inactive)
                    )
                    binding.tvStats.text = "${status.sessions_today} sessions · ${status.people_today} people today"
                    val onWifi = isOnWifi()
                ...
                withContext(Dispatchers.Main) { binding.tvStats.text = "Server unreachable" }
            }
        }
    }
